...
import { ShieldCheck, Users, Sparkles, MapPin, Search, MessageSquare, Star, Calendar, Bot, Send, Loader2, AlertCircle } from "lucide-react";
...
export default function Home() {
  ...
  const handleAiAsk = async () => {
    ...
    try {
      ...
      setAiResponse("عذراً، واجهت مشكلة في الاتصال بالمساعد الذكي حالياً. يرجى مراجعة إعدادات الـ API Key في الـ Backend.");
    } finally {
      ...
      <main className="flex-1">
        ...
        <section className="py-16 bg-muted/20 border-b" dir="rtl">
          <div className="container px-4 md:px-8 max-w-4xl mx-auto">
            <div ref={reveal} className="anim-ready bg-gradient-to-br from-zinc-900 via-card to-zinc-900 border border-primary/20 rounded-[2.5rem] p-6 md:p-8 shadow-2xl relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-xl font-heading font-black text-white">المساعد الطبي الذكي لـ "صحتي"</h3>
                    <span className="px-2 py-0.5 bg-primary/20 text-primary border border-primary/30 rounded-full text-[10px] font-bold">ميزة ثورية</span>
                  </div>
                  <p className="text-zinc-400 text-xs mt-0.5">هل أنت حائر ولا تعرف أي تخصص طبي تحتاج؟ اشرح أعراضك هنا وسأوجهك فوراً.</p>
                </div>
              </div>
                ...
                {aiResponse && (
                  <div className="bg-zinc-950/80 border border-white/5 rounded-2xl p-4 text-sm text-zinc-200 leading-relaxed space-y-3 animate-fade-in">
                    ...
                    <div className="pt-2 border-t border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <Button size="sm" onClick={() => setLocation("/doctors")} className="text-xs font-black rounded-xl h-8 shrink-0">
