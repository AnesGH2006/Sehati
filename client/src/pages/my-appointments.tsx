        ...
        import {
          ...
          Plus, X, Check, MessageSquare, Stethoscope, ChevronDown,
        } from "lucide-react";
        ...
        // ── ثوابت ─────────────────────────────────────────────────────────────────────
        const STATUS_MAP: Record<AppointmentStatus, { label: string; color: string; icon: React.ReactNode }> = {
          pending:   { label: "بانتظار التأكيد", color: "bg-yellow-500/20 text-yellow-300 border-yellow-500/30",  icon: <Clock className="h-3 w-3" /> },
          confirmed: { label: "مؤكد",            color: "bg-green-500/20 text-green-300 border-green-500/30",    icon: <CheckCircle2 className="h-3 w-3" /> },
          completed: { label: "منتهي",           color: "bg-zinc-500/20 text-zinc-300 border-zinc-500/30",       icon: <Check className="h-3 w-3" /> },
          cancelled: { label: "ملغي",            color: "bg-red-500/20 text-red-300 border-red-500/30",          icon: <X className="h-3 w-3" /> },
        };
        ...
        export default function MyAppointments() {
          ...
          useEffect(() => {
            if (!appointments.length || !doctors.length) { setEnriched(appointments); return; }
            const enrichedList = appointments.map(a => {
              const doc = doctors.find((d: any) => d.id === a.doctorId);
              return { ...a, doctorName: doc?.name || `طبيب #${a.doctorId}`, doctorSpecialty: doc?.specialty || "" };
            });
          ...
          const cancelMutation = useMutation({
            mutationFn: (id: number) => fetch(`/api/appointments/${id}/status`, {
              ...
              <div className="min-h-screen flex flex-col bg-background">
                ...
                <main className="flex-1 flex items-center justify-center">
                  <div className="text-center space-y-4">
                    ...
                    <p className="text-muted-foreground">يجب تسجيل الدخول كمريض للوصول لمواعيدك</p>
          ...
          const filtered = activeTab === "all" ? enriched : enriched.filter(a => a.status === activeTab);
            ...
            <div className="min-h-screen flex flex-col bg-background" dir="rtl">
              ...
              <main className="flex-1 container max-w-4xl mx-auto px-4 py-8">
                ...
                <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                  ...
                  <Button onClick={() => setShowBooking(true)} className="gap-2 rounded-xl font-bold">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                  {[
                    { label: "إجمالي المواعيد",     value: stats.total,     icon: <Calendar className="h-4 w-4" />,     color: "text-blue-500" },
                    { label: "بانتظار التأكيد",     value: stats.pending,   icon: <Clock className="h-4 w-4" />,        color: "text-yellow-500" },
                    { label: "مواعيد مؤكدة",        value: stats.confirmed, icon: <CheckCircle2 className="h-4 w-4" />, color: "text-green-500" },
                    { label: "مواعيد منتهية",       value: stats.completed, icon: <Check className="h-4 w-4" />,        color: "text-zinc-400" },
                  ].map(s => (
                ...
                {isLoading ? (
                  ...
                  <div className="text-center py-20 text-muted-foreground">
                    ...
                    <Button variant="link" onClick={() => setShowBooking(true)} className="mt-2">احجز موعدك الأول</Button>
                  </div>
                ) : (
                  <AnimatePresence>
                    <div className="space-y-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-3 flex-wrap text-sm text-muted-foreground mb-2">
                              <span className="flex items-center gap-1">
                                ...
                                {new Date(appt.appointmentDate).toLocaleDateString("ar-DZ", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
                              </span>
                              <span className="flex items-center gap-1">
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {appt.notes && (
                                <span className="text-xs text-muted-foreground italic">"{appt.notes}"</span>
                              )}
                            </div>

                            {appt.doctorNotes && (
                              <div className="mt-2 text-xs bg-muted/50 rounded-lg px-3 py-2 text-muted-foreground border border-border/30">
                                <span className="font-medium text-foreground">ملاحظة الطبيب: </span>{appt.doctorNotes}
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex gap-2 shrink-0">
                            ...
                            {(appt.status === "pending" || appt.status === "confirmed") && (
                              <Button size="sm" variant="destructive" className="gap-1 rounded-xl text-xs"
                                onClick={() => { if (confirm("هل تريد إلغاء هذا الموعد؟")) cancelMutation.mutate(appt.id); }}
              ...
              <BookingDialog
                ...
                onSuccess={() => {
                  ...
                  toast({ title: "✅ تم حجز موعدك بنجاح", description: "انتظر تأكيد الطبيب" });
                }}
              />
            </div>
          );
        ...
        // ── نافذة الحجز ───────────────────────────────────────────────────────────────
        function BookingDialog({ open, onClose, doctors, patientId, patientName, onSuccess }: {
          open: boolean;
          onClose: () => void;
          doctors: any[];
          patientId: string;
          ...
          const bookMutation = useMutation({
            mutationFn: () => fetch("/api/appointments", {
            ...
            onError:    () => toast({ title: "فشل الحجز، حاول مرة أخرى", variant: "destructive" }),
          });

          const resetForm = () => {
            setDoctorId(""); setPhone(""); setDate(new Date().toISOString().slice(0, 10));
            setSelectedSlot(""); setNotes(""); setIsUrgent(false); setSlots([]);
          };
              ...
              <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto" dir="rtl">
                ...
                <div className="space-y-4 py-2">
                  ...
                  {selectedDoctor && (
                    <div className="bg-muted/40 rounded-xl p-3 text-sm border border-border/50">
                      <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        {selectedDoctor.clinicName && <span>• {selectedDoctor.clinicName}</span>}
                      </div>
                      {selectedDoctor.clinicAddress && <p className="text-xs text-muted-foreground mt-1">📍 {selectedDoctor.clinicAddress}</p>}
                      {selectedDoctor.consultationFee && (
                        <p className="text-xs text-muted-foreground mt-0.5">💰 سعر الكشف: {selectedDoctor.consultationFee} دج</p>
                      )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <Label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">اسمك</Label>
                      <Input value={patName} onChange={e => setPatName(e.target.value)} placeholder="الاسم الكامل" className="h-11 rounded-xl" />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">الهاتف</Label>
                  ...
                  {/* التاريخ */}
                  <div className="space-y-1.5">
                    <Label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">تاريخ الموعد</Label>
                    <Input type="date" value={date} min={today} onChange={e => setDate(e.target.value)} className="h-11 rounded-xl" />
                  </div>

                  {/* الأوقات المتاحة */}
                  {doctorId && date && (
                    <div className="space-y-1.5">
                      <Label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">الوقت المتاح</Label>
                      {loadingSlots ? (
                        <div className="text-center py-4 text-sm text-muted-foreground">جاري تحميل الأوقات...</div>
                      ) : slots.length === 0 ? (
                  ...
                  {/* الملاحظات */}
                  <div className="space-y-1.5">
                    <Label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">ملاحظات (اختياري)</Label>
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-red-500/5 border border-red-500/20">
                    <button onClick={() => setIsUrgent(p => !p)}
                      ...
                      <span className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${isUrgent ? "right-1" : "right-5"}`} />
                    </button>
                    <div>
                      <p className="text-sm font-medium">طلب عاجل</p>
                      <p className="text-xs text-muted-foreground">سيتم إشعار الطبيب فوراً</p>
                    </div>
                    {isUrgent && (
                  ...
                  <div className="flex gap-3 pt-2">
                    <Button variant="outline" onClick={handleClose} className="flex-1 rounded-xl">إلغاء</Button>
                    <Button
                      onClick={() => bookMutation.mutate()}
                      disabled={!doctorId || !patName || !date || !selectedSlot || bookMutation.isPending}
