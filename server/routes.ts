...
export async function registerRoutes(
  ...
  app.post("/api/auth/register", async (req: Request, res: Response) => {
    try {
      ...
      if (password.length < 6)
        return res
          .status(400)
          .json({ message: "كلمة المرور يجب ان تكون 6 احرف على الاقل" });
  ...
  app.post("/api/auth/verify-email", async (req: Request, res: Response) => {
    try {
      ...
      if (!verified)
        return res
          .status(400)
          .json({ message: "رمز التحقق غير صحيح او انتهت صلاحيته" });
  ...
  app.post("/api/auth/resend-otp", async (req: Request, res: Response) => {
    try {
      ...
      if (user.isVerified)
        return res.status(400).json({ message: "الحساب مؤكد بالفعل" });
      const otp = generateOTP();
      await storage.setUserOTP(email, otp);
      const sent = await sendVerificationEmail(email, user.name, otp);
      res.json({
        success: sent,
        message: sent ? "تم ارسال رمز جديد" : "فشل الارسال",
      });
    } catch {
      res.status(500).json({ message: "خطا في الخادم" });
    }
  ...
  app.post("/api/auth/forgot-password", async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        success: sent,
        message: sent ? "تم ارسال رمز اعادة التعيين" : "فشل الارسال",
      });
    } catch {
      res.status(500).json({ message: "خطا في الخادم" });
    }
  ...
  app.post("/api/doctors", async (req: Request, res: Response) => {
    console.log("🔥 POST /api/doctors hit");
  ...
  app.get("/api/doctors/:id/analytics", async (req: Request, res: Response) => {
    try {
      ...
      const [convList, reviewList, viewStats, appointmentList] =
        await Promise.all([
      ...
      const convsThisMonth = convList.filter(
        (c: any) => new Date(c.createdAt) >= startOfThisMonth,
      ).length;
      const convsLastMonth = convList.filter(
        (c: any) =>
          new Date(c.createdAt) >= startOfLastMonth &&
          new Date(c.createdAt) < startOfThisMonth,
      ).length;
      const reviewsThisMonth = reviewList.filter(
        (r: any) => new Date(r.createdAt) >= startOfThisMonth,
      ).length;
      const reviewsLastMonth = reviewList.filter(
        (r: any) =>
          new Date(r.createdAt) >= startOfLastMonth &&
          new Date(r.createdAt) < startOfThisMonth,
      ).length;
      const apptThisMonth = appointmentList.filter(
        (a: any) => new Date(a.createdAt) >= startOfThisMonth,
      ).length;
      const apptLastMonth = appointmentList.filter(
        (a: any) =>
          new Date(a.createdAt) >= startOfLastMonth &&
          new Date(a.createdAt) < startOfThisMonth,
      ).length;
      ...
      const replyRate =
        totalConvs > 0
          ? Math.round(
              (convList.filter((c: any) => c.lastMessage).length / totalConvs) *
                100,
            )
          : 100;
      const avgRating =
      ...
      res.json({
        ...
        viewsThisMonth: viewStats.thisMonth,
        viewsLastMonth: viewStats.lastMonth,
        viewsChange: pct(viewStats.thisMonth, viewStats.lastMonth),
        dailyViews: viewStats.daily,
        totalConversations: totalConvs,
        convsThisMonth,
        convsLastMonth,
        convsChange: pct(convsThisMonth, convsLastMonth),
        avgRating: Math.round(avgRating * 10) / 10,
        totalReviews: reviewList.length,
        reviewsThisMonth,
        reviewsLastMonth,
        reviewsChange: pct(reviewsThisMonth, reviewsLastMonth),
        totalAppointments: appointmentList.length,
        apptThisMonth,
        apptLastMonth,
        apptChange: pct(apptThisMonth, apptLastMonth),
        completedAppointments,
        replyRate,
        dailyConversations: dailyConvs,
        monthlyConversations: monthlyConvs,
        ratingDistribution: ratingDist,
        improvements: [
          {
            key: "views",
            label: "المشاهدات",
            value: Math.min(Math.round((viewStats.thisMonth / 100) * 100), 100),
            change: pct(viewStats.thisMonth, viewStats.lastMonth),
            color: "#3B82F6",
          },
          {
            key: "appointments",
            label: "المواعيد",
            value: Math.min(Math.round((apptThisMonth / 20) * 100), 100),
            change: pct(apptThisMonth, apptLastMonth),
            color: "#8B5CF6",
          },
          {
            key: "rating",
            label: "التقييم",
            value: Math.round((avgRating / 5) * 100),
            change: pct(reviewsThisMonth, reviewsLastMonth),
            color: "#F59E0B",
          },
          {
            key: "replyRate",
          ...
          },
          {
            ...
            value:
              appointmentList.length > 0
                ? Math.round(
                    (completedAppointments / appointmentList.length) * 100,
                  )
                : 0,
            change: 0,
            color: "#EC4899",
      ...
      res.status(500).json({ error: "فشل جلب الاحصاءات" });
    }
  });
  ...
  app.get("/api/doctors/:id/reviews", async (req: Request, res: Response) => {
    try {
      res.json(await storage.getReviewsByDoctor(parseInt(req.params.id)));
    } catch {
  ...
  app.post("/api/reviews", async (req: Request, res: Response) => {
    try {
      const data = insertReviewSchema.parse(req.body) as {
        doctorId: number;
        patientId: string;
      };
      if (await storage.hasReviewed(data.doctorId, data.patientId))
        return res.status(409).json({ message: "لقد قيمت هذا الطبيب من قبل" });
  ...
  app.delete("/api/reviews/:id", async (req: Request, res: Response) => {
    if (!isAdmin(req)) return res.status(403).json({ message: "Forbidden" });
    try {
      const d = await storage.deleteReview(parseInt(req.params.id));
      if (!d) return res.status(404).json({ message: "Review not found" });
      res.json({ success: true });
    } catch {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        res.json(
          await storage.getAppointmentsByDoctor(parseInt(req.params.doctorId)),
        );
      } catch (err) {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        res.json(await storage.getAppointmentsByPatient(req.params.patientId));
      } catch (err) {
        console.error("[appointments]", err);
        res.status(500).json({ message: "Failed to fetch appointments" });
      }
  ...
  app.get("/api/conversations/:userId", async (req: Request, res: Response) => {
    try {
      ...
      console.error("[conversations]", err?.message, err?.stack);
      res
        .status(500)
  ...
  app.delete("/api/messages/:id", async (req: Request, res: Response) => {
    try {
      const d = await storage.deleteMessage(parseInt(req.params.id));
      if (!d) return res.status(404).json({ message: "Message not found" });
      res.json({ success: true });
    } catch {
  ...
  app.patch("/api/messages/:id", async (req: Request, res: Response) => {
    try {
      ...
      const msg = await storage.editMessage(parseInt(req.params.id), content);
...
async function sendPushNotification(data: any): Promise<void> {
  const body = data.content.startsWith("data:image")
    ? "📷 صورة"
